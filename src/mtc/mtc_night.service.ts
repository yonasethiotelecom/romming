...
export class MtcNightService {
  ...
  async readAllCsvFilesrFomFtpServer(): Promise<any[]> {
    ...
    try {
      ...
      const remoteDirectory = '/Mtc/night'; // Specify the remote directory path here
        ...
        if (
          ...
          for await (const data of csvStream) {
            const formattedDataH = {
              ...
              'Period (min)': data[1],
              'NE Name': data[2],
              'Incoming Office': data[3],
              'Calling Number': data[4],
              'Outgoing Office': data[5],
              'Called Number': data[6],
              CallType: data[7],
              'CALL ATTEMPT TIMES (times)': data[8],
              'NORMAL CALL CLEAR (times)': data[9],
              'RELEASE BEFORE RING (times)': data[10],
              '{UDC} ASR (None)': data[11],
              'ANSWER TIMES (times)': data[12],
            };
          ...
          if (!existingfileName) {
            ...
            await this.prisma.mtc_Asr.create({
              data: {
                ASR: await moveLastElementToFourthPosition(convertToList(ASR)),
                shift: 'night',
          ...
          const fileObject = {
            fileName: file.name + 'night',
            ASR: moveLastElementToFourthPosition(convertToList(ASR)),
          };
...
function aggregateSite(data: any, s: string) {
  ...
  data.forEach((item) => {
    const site = item['Outgoing Office'];
...
function moveLastElementToFourthPosition(arr) {
  if (arr.length < 4) {
    console.warn('Array length is less than 4, no changes made.');
    return arr;
  }
  ...
  // Insert the last element at the fourth position (index 3)
  arr.splice(5, 0, lastElement);
