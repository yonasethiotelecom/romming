...
@Injectable()
export class MtcReadingService {
  constructor(private readonly prisma: PrismaService) {}

  findTable(startDate: Date, endDate: Date, day: Jobshift) {
    return this.prisma.mtc_Asr.findMany({
      where: {
        shift: day,
        createdAt: {
          gte: startDate,
  ...
  async findLine(startDate: Date, endDate: Date, day: Jobshift) {
    const nivoline = await this.prisma.mtc_Asr.findMany({
      where: {
        shift: day,
        createdAt: {
          gte: startDate,
  ...
  async findASRBar(startDate: Date, endDate: Date, day: Jobshift) {
    const ASRBAR = await this.prisma.mtc_Asr.findMany({
      where: {
        shift: day,
        createdAt: {
          gte: startDate,
  ...
  findAll() {
    return `This action returns all mtcReading`;
  }

  findOne(id: number) {
    return `This action returns a #${id} mtcReading`;
  }

  remove(id: number) {
    return `This action removes a #${id} mtcReading`;
  }
}
